...
multilevel_fragment_subject_var_slope_model_time_error2 <- map2stan(
    alist(
        ...
        # Account for error in time measurements (convert to time since infection)
        time ~ dnorm(infection_time - time_error, 0.1),
        ...
        subject_time_mean ~ dnorm(0, 0.1) & T[0,], 
        subject_time_sd ~ dcauchy(0,0.1),
        subject_slope_mean ~ dnorm(0,1),
        subject_slope_sd ~ dcauchy(0,20),
        population_avg_slope ~ dunif(0, 150),
...
save_models("multilevel_fragment_subject_var_slope_model_time_error2")
...
save_models("multilevel_fragment_subject_var_slope_model_time_error2_var_int")
...
multilevel_fragment_subject_var_slope_model_time_error2_var_int_multivar_norm <- map2stan(
    ...
    data = indexed_infant_data_cleaned_subset, start = list(infection_time = indexed_infant_data_cleaned_subset$time +0.2), init_r = 0.5, WAIC = FALSE, iter = 30000, warmup = 15000, chains = 3, cores = 3, control = list(max_treedepth = 20, adapt_delta= 0.99)
)

save_models("multilevel_fragment_subject_var_slope_model_time_error2_var_int_multivar_norm")


# This incorporates time error for subject specific time error (accounting for different infection times)
multilevel_fragment_subject_var_slope_model_time_error3 <- map2stan(
    alist(
        # residuals
        time_since_infection ~ dnorm(population_mean, population_sd), 
        population_sd ~ dcauchy(0,0.5),
        ...
        # Account for error in time measurements (convert to time since infection)
        time ~ dcauchy(time_since_infection - time_error, 0.05),

        time_error <- subject_time_error[subject_index] & T[0,],

        # adaptive priors
        subject_slope[subject_index] ~ dnorm(subject_slope_mean, subject_slope_sd), 
        fragment_slope[frag_index] ~ dnorm(fragment_slope_mean, fragment_slope_sd), 
        subject_time_error[subject_index] ~ dunif(time_point1, time_point2),

        # fixed priors
        time_point1 ~ dnorm(0, 0.1) & T[0,0.125],
        time_point2 ~ dnorm(0.125, 0.1) & T[0.125,0.75],
        subject_slope_mean ~ dnorm(0,1),
        subject_slope_sd ~ dcauchy(0,20),
        population_avg_slope ~ dunif(0, 150),
        fragment_slope_mean ~ dnorm(0,1),
    ...
    data = indexed_infant_data_cleaned_subset, start = list(time_since_infection = indexed_infant_data_cleaned_subset$time + 0.2), init_r = 0.5, WAIC = FALSE, iter = 30000, warmup = 10000, chains = 1, cores = 1, control = list(max_treedepth = 15, adapt_delta= 0.99)
)
...
multilevel_fragment_subject_var_slope_model_time_error3 <- map2stan(
    alist(
        ...
        #time_point1 ~ dnorm(0, 0.1) & T[0,0.125],
        #time_point2 ~ dnorm(0.125, 0.1) & T[0.125,0.75],
        subject_time_mean ~ dunif(0, 0.75), 
        subject_time_sd ~ dcauchy(0,1),
        subject_slope_mean ~ dnorm(0,1),
        subject_slope_sd ~ dcauchy(0,20),
        population_avg_slope ~ dunif(0, 150),
        fragment_slope_mean ~ dnorm(0,1),
...
multilevel_fragment_subject_var_slope_model_time_error4 <- map2stan(
    alist(
        # residuals
        time_since_infection ~ dnorm(population_mean, population_sd), 
        population_sd ~ dcauchy(0,0.5),
