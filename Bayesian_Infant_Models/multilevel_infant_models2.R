...
# No divergent iterations here!
multilevel_fragment_subject_var_slope_model_new_time_error <- map2stan(
    alist(
        # residuals
        time ~ dnorm(time_since_infection, time_error),
        time_since_infection ~ dnorm(population_mean, population_sd), 
        time_error ~ dunif(0,0.125),
        population_sd ~ dcauchy(0,0.5),
        # linear models
        population_mean <- total_slope*apd,
        total_slope <- (population_avg_slope + subject_slope[subject_index] + fragment_slope[frag_index]) & T[0,],
...
save_models_plots_stats("multilevel_fragment_subject_var_slope_model_new_time_error", type = "fragment_subject_varying_slopes_no_int_time_error")