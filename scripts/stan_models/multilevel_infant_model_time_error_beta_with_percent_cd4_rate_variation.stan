data{
    ...
    real percent_cd4rate[observation_count];
}
parameters{
    ...
    real percent_cd4rate_slope_delta;   // slope before subject/fragment slope changes
    real percent_cd4rate_slope_delta_mean_estimate;
    real<lower=0> percent_cd4rate_slope_delta_variance_estimate;
...
model{
    ...
    percent_cd4rate_slope_delta_mean_estimate ~ normal( 0 , 1 );
    percent_cd4rate_slope_delta_variance_estimate ~ cauchy( 0 , 20 );
    percent_cd4rate_slope_delta ~ normal( percent_cd4rate_slope_delta_mean_estimate , percent_cd4rate_slope_delta_variance_estimate );
    ...
    for ( i in 1:observation_count ) {
        predicted_time_since_infection[i] = total_slope[i] * apd[i] + percent_cd4rate_slope_delta * apd[i]*percent_cd4rate[i]; // linear function relating apd to time_since_infection
    }
...
generated quantities{
    ...
    for ( i in 1:observation_count ) {
        predicted_time_since_infection[i] = total_slope[i] * apd[i] + percent_cd4rate_slope_delta * apd[i] * percent_cd4rate[i];
    }
    for ( i in 1:observation_count ) {
        if ( is_utero[i] ){
          observed_time_to_time_since_infection_correction[subject[i]] = observed_time_to_time_since_infection_correction_utero[subject[i]];
