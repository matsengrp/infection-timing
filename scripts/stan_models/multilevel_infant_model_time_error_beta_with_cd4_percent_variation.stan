data{
    ...
    real avg_cd4[observation_count];
}
parameters{
    ...
    real avg_cd4_slope_delta;   // slope before subject/fragment slope changes
    real avg_cd4_slope_delta_mean_estimate;
    real<lower=0> avg_cd4_slope_delta_variance_estimate;
...
model{
    ...
    avg_cd4_slope_delta_mean_estimate ~ normal( 0 , 1 );
    avg_cd4_slope_delta_variance_estimate ~ cauchy( 0 , 20 );
    avg_cd4_slope_delta ~ normal( avg_cd4_slope_delta_mean_estimate , avg_cd4_slope_delta_variance_estimate );
    ...
    for ( i in 1:observation_count ) {
        predicted_time_since_infection[i] = total_slope[i] * apd[i] + avg_cd4_slope_delta * apd[i]*avg_cd4[i]; // linear function relating apd to time_since_infection
    }
...
generated quantities{
    ...
    for ( i in 1:observation_count ) {
        predicted_time_since_infection[i] = total_slope[i] * apd[i] + avg_cd4_slope_delta * apd[i] * avg_cd4[i];
    }
    for ( i in 1:observation_count ) {
        if ( is_utero[i] ){
          observed_time_to_time_since_infection_correction[subject[i]] = observed_time_to_time_since_infection_correction_utero[subject[i]];
