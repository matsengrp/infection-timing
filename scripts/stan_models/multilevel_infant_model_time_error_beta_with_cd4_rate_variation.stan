data{
    ...
    real cd4rate[observation_count];
}
parameters{
    ...
    real cd4rate_slope_delta;   // slope before subject/fragment slope changes
    real cd4rate_slope_delta_mean_estimate;
    real<lower=0> cd4rate_slope_delta_variance_estimate;
...
model{
    ...
    cd4rate_slope_delta_mean_estimate ~ normal( 0 , 1 );
    cd4rate_slope_delta_variance_estimate ~ cauchy( 0 , 20 );
    cd4rate_slope_delta ~ normal( cd4rate_slope_delta_mean_estimate , cd4rate_slope_delta_variance_estimate );
    ...
    for ( i in 1:observation_count ) {
        predicted_time_since_infection[i] = total_slope[i] * apd[i] + cd4rate_slope_delta * apd[i]*cd4rate[i]; // linear function relating apd to time_since_infection
    }
...
generated quantities{
    ...
    for ( i in 1:observation_count ) {
        predicted_time_since_infection[i] = total_slope[i] * apd[i] + cd4rate_slope_delta * apd[i] * cd4rate[i];
    }
    for ( i in 1:observation_count ) {
        if ( is_utero[i] ){
          observed_time_to_time_since_infection_correction[subject[i]] = observed_time_to_time_since_infection_correction_utero[subject[i]];
