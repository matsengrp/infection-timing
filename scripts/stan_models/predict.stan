data{
    ...
    matrix[iteration_count, fragment_count] fragment_slope_delta_reparameterized;
    ...
    for (subj in 1:subject_count){
        for (iter in 1:iteration_count){
          ...
          for (i in 1:observation_count){
              total_slope[iter, i] = (baseline_slope[iter] + subject_slope_delta[iter, subj] + fragment_slope_delta_reparameterized[iter, fragment[i]]);
