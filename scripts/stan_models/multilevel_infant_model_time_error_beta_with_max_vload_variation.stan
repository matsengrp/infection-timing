data{
    ...
    real max_vload[observation_count];
}
parameters{
    ...
    real max_vload_slope_delta;   // slope before subject/fragment slope changes
    real max_vload_slope_delta_mean_estimate;
    real<lower=0> max_vload_slope_delta_variance_estimate;
...
model{
    ...
    max_vload_slope_delta_mean_estimate ~ normal( 0 , 1 );
    max_vload_slope_delta_variance_estimate ~ cauchy( 0 , 20 );
    max_vload_slope_delta ~ normal( max_vload_slope_delta_mean_estimate , max_vload_slope_delta_variance_estimate );
    ...
    for ( i in 1:observation_count ) {
        predicted_time_since_infection[i] = total_slope[i] * apd[i] + max_vload_slope_delta * apd[i]*max_vload[i]; // linear function relating apd to time_since_infection
    }
...
generated quantities{
    ...
    for ( i in 1:observation_count ) {
        predicted_time_since_infection[i] = total_slope[i] * apd[i] + max_vload_slope_delta * apd[i] * max_vload[i];
    }
    for ( i in 1:observation_count ) {
        if ( is_utero[i] ){
          observed_time_to_time_since_infection_correction[subject[i]] = observed_time_to_time_since_infection_correction_utero[subject[i]];
