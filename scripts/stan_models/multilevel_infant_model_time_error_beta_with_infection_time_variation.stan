data{
    ...
    int<lower=1> infection_time_count;  // number of infection time statuses (Early/Late)
    ...
    int infection_time[observation_count];
...
parameters{
    ...
    vector[infection_time_count] infection_time_slope_delta;  // vector of infection time specific slope changes
    real infection_time_slope_delta_mean_estimate;    // infection time specific slope change mean
    real<lower=0> infection_time_slope_delta_variance_estimate; // infection time specific slope change standard deviation
}
...
model{
    ...
    infection_time_slope_delta_mean_estimate ~ normal( 0 , 1 );
    infection_time_slope_delta_variance_estimate ~ cauchy( 0 , 20 );
    infection_time_slope_delta ~ normal( infection_time_slope_delta_mean_estimate , infection_time_slope_delta_variance_estimate );
    for ( i in 1:observation_count ) {
        total_slope[i] = (baseline_slope + subject_slope_delta[subject[i]] + fragment_slope_delta_reparameterized[fragment[i]] + infection_time_slope_delta[infection_time[i]]);
    }
    for ( i in 1:observation_count ) {
