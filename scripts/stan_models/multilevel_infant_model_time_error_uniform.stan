...
parameters{
    ...
    vector<lower=-1, upper=-0.08333>[subject_count] observed_time_to_time_since_infection_correction_post;    // conversion factor between observed time (age at sampling time) measurements and time since infection output
...
model{
    ...
    observed_time_to_time_since_infection_correction_utero ~ uniform(0,0.75);
    observed_time_to_time_since_infection_correction_post ~ uniform(-1, -0.08333);
    ...
    for ( i in 1:observation_count ) {
        if ( is_utero[i] ){
          ...
          observed_time_to_time_since_infection_correction[subject[i]] = observed_time_to_time_since_infection_correction_post[subject[i]];
        }
    }
    for ( i in 1:observation_count ) {
