...
mae = calculate_mae_dt(posterior_means)

plot_scatter = plot_observed_predicted_time(posterior_means, FALSE, TRUE, with_subject_legend = TRUE, with_fragment_legend = FALSE)
plot_scatter_loocv = plot_scatter + facet_grid(~fragment) + labs(color = 'ptnum') 
ggsave(paste0('plots/manuscript_figs/infant_model_loocv_scatter_by_frag.pdf'), plot = plot_scatter_loocv, width = 27, height = 9.5, units = 'in', dpi = 750, device = cairo_pdf)

plot_hist_loocv = plot_observed_predicted_time_histogram(posterior_means, FALSE, TRUE)+
    geom_text(data = mae, x = 2, y = Inf, aes(label = paste0('MAE = ', mae)), vjust = 2, size = 8)
ggsave(paste0('plots/manuscript_figs/infant_model_loocv_hist_by_frag.pdf'), plot = plot_hist_loocv, width = 13, height = 11.25, units = 'in', dpi = 750, device = cairo_pdf)
