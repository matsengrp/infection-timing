...
title <- function(apd, fragment, run, facet1, facet2) {
...
file <- function(apd, fragment, run, facet1, facet2) {
...
plot_ETI_TI_regression <- function(data, fragment, apd, run, facet1, facet2, points) {
    # Set title and file name 
    title_name = title(apd, fragment, run, facet1, facet2)
    file_name = file(apd, fragment, run, facet1, facet2)
    ...
    if (facet2 == "none"){
        # Create plot
        p <- ggplot(data=data_arg,aes(data_arg[[avgETI]],x=ActualTOI..year.))+geom_point(aes(shape=get(paste(points))), size = 3)+stat_smooth(method="lm",    se=TRUE, color= apd, fill= apd) + labs(x="Actual Time of Infection (years)", y="Average Estimated Time of Infection (years)")+ xlim(0,2)    + ylim(0,6) + facet_wrap(as.formula(paste("~", facet1))) + ggtitle(title_name) + stat_regline_equation(label.x = 0, label.y = 6, size = 7)   + labs(shape = points, color = "Exact timing correlation") + stat_function(fun = func, color = "blue", linetype="dashed", size=1) + theme_bw() + theme(text = element_text(size = 22)) 
    } else {
        # Create plot
        p <- ggplot(data=data_arg,aes(data_arg[[avgETI]],x=ActualTOI..year.))+geom_point(aes(shape=get(paste(points))), size = 2)+stat_smooth(method="lm",    se=TRUE, color= apd, fill= apd) + labs(x="Actual Time of Infection (years)", y="Average Estimated Time of Infection (years)")+ xlim(0,2)    + ylim(0,6) + facet_wrap(as.formula(paste(facet1, "~", facet2))) + ggtitle(title_name) + stat_regline_equation(label.x = 0, label.y = 5, size = 5) + labs(shape = points, color = "Exact timing correlation") + stat_function(fun = func, color = "blue", linetype="dashed", size=1) + theme_bw() + theme(legend.position = "none", text = element_text(size = 18)) 
...
make_all_plots <- function(){
    ...
    for (diversity in c(1, 5, 10)){
        plot_ETI_TI_regression(all_runs, fragment = "all", apd = diversity, run = "all", facet1 = "Fragment", facet2 = "Run", points = "Run")
        plot_ETI_TI_regression(all_runs, fragment = "all", apd = diversity, run = "all", facet1 = "Fragment", facet2 = "none", points = "Run")
    }
}
```
# The following plots show the estimated time of infection(ETI) (as calculated using the Neher equation) versus the actual time of infection (TI) (time since birth). Note that average pairwise diversity (APD) parameters change for each plot (color of regression line indicates different APD cutoff). Note that we are also varying runs analyzed and fragments analyzed. The blue dotted line for each plot represents the exact timing correlation (i.e. ETI = TI)...
