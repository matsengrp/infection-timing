...
plot_ETI_TI_regression <- function(data, fragment, apd, run, facet1, facet2, points, color_points, y) {
    ...
    if (y == "eti"){
        ...
        # Create plot
        p <- ggplot(data=data_arg,aes(y = data_arg[[yvar]],x=data_arg[[xvar]], color = get(paste(color_points)), group = get(paste(color_points)))) + geom_point(aes(color=get(paste(color_points))), size = 2)+ facet_wrap(facet_wrap)+ geom_smooth(se = FALSE, method = "lm") + labs(x=paste(xlab), y=paste(ylab)) + ggtitle(title_name) + labs(color = color_points) + theme_bw() + theme(text = element_text(size = 18)) 
    }
...
make_all_plots <- function(variable){
    ...
    if (variable == "vl"){
        for (diversity in c(1, 5, 10)){
            plot_ETI_TI_regression(all_runs, fragment = "all", apd = diversity, run = "all", facet1 = "Fragment", facet2 = "none",points="none", color_point = "Sample", y = paste(variable))
        }
    } else {
        for (diversity in c(1, 5, 10)){
